...
def main(params, kt_name, testmodel, lr):
    ...
    for i in range(1, 11):
        with open("../configs/data_config.json") as fin:
            ...
            # 更换kt时这里也要换
            data_config["test_file"] = f'model/{testmodel}/{kt_name}/Topk_insert_data/lr-{lr}/top{i}_insert_testdata.csv'
        ...
        os.makedirs(os.path.join(dpath,f'model/{testmodel}/{kt_name}/lr-{lr}'), exist_ok=True)
        torch.save(stu_ks, os.path.join(dpath, f'model/{testmodel}/{kt_name}/lr-{lr}/top{i}_pkc.pth'))
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    # kt_name = 'atkt' if d == 'nips_task34' else 'dkt_forget'
    for kt_name in kt_names:
        for lr in ['0.001']:
            args = parser.parse_args()
            print(args)
            params = vars(args)
            main(params, kt_name, testmodel, lr)

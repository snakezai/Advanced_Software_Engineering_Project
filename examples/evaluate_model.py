    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["atdkt"]:
                ...
                y = model(c.long(), r.long())  # y --> [batch, seq, kc_num], sm --> [batch, seq]
               
                for i in range(len(sm)):
                    stu_kc = y[i]
                    mask = sm[i]
                    index_0_list = mask.nonzero()
                    if index_0_list.numel() > 0:
                        index = torch.max(index_0_list).item()
                    else:
                        index = len(mask) - 1
                    stu_ks.append(y[i, index, :])

                    # print(y[i, index, :])
                    # print(y[i, index - 1, :])
                    # start = y.sum(dim=2)

                # print(cshft.min(), cshft.max())
                y = (y * one_hot(cshft.long(), model.num_c)).sum(-1)

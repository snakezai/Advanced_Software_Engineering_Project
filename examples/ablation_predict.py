...
def main(params, kt_name, testmodel, ablation):
    ...
    for i in range(1, 11):
        with open("../configs/data_config.json") as fin:
            ...
            # 更换kt时这里也要换
            data_config["test_file"] = f'model/{testmodel}/{kt_name}/Topk_insert_data/{ablation}/top{i}_insert_testdata.csv'
        ...
        os.makedirs(os.path.join(dpath, f'model/{testmodel}/{kt_name}/{ablation}'), exist_ok=True)
        torch.save(stu_ks, os.path.join(dpath, f'model/{testmodel}/{kt_name}/{ablation}/top{i}_pkc.pth'))
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    # for ablation in ['without_long', 'without_short', 'only_exercise', 'only_short']:
    for ablation in ['only_short']:
        args = parser.parse_args()
        print(args)
        params = vars(args)
        main(params, kt_name, testmodel, ablation)

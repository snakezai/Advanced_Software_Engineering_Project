...
def main(params, kt_name, testmodel, learning_ratio):
    ...
    for i in range(1, 11):
        with open("../configs/data_config.json") as fin:
            ...
            # 更换kt时这里也要换
            data_config["test_file"] = f'model/{testmodel}/{kt_name}/Topk_insert_data/lr-{learning_ratio}/top{i}_insert_testdata.csv'
        ...
        os.makedirs(os.path.join(dpath, f'model/{testmodel}/{kt_name}/lr-{learning_ratio}'), exist_ok=True)
        torch.save(stu_ks, os.path.join(dpath, f'model/{testmodel}/{kt_name}/lr-{learning_ratio}/top{i}_pkc.pth'))
...
if __name__ == "__main__":
    testmodel = 'LP4Rec'
    # 测试前先设好kt的名字
    # kt_names = ['dkt', 'dkt+', 'dkt_forget', 'atkt', 'random']
    kt_names = ['random']
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    for kt_name in kt_names:
        ...
        print(args)
        params = vars(args)
        main(params, kt_name, testmodel, learning_ratio)

...
from examples.evaluate_model import evaluate
...
def main(params, testmodel):
    ...
    with open("./configs/ks_get_congfig.json") as fin:
        curconfig = copy.deepcopy(json.load(fin))
        data_config = curconfig[dataset_name]
        data_config["dataset_name"] = dataset_name

        # 测试集位置
        data_config["test_file"] = f'model/{testmodel}/test_sequences.csv'
    ...
    # torch.save(stu_ks, os.path.join(save_dir, "pkm.pth"))
    os.makedirs(os.path.join(dpath, f'model/{testmodel}'), exist_ok=True)
    torch.save(stu_ks, os.path.join(dpath, f"model/{testmodel}/ori_pkc.pth"))

    # os.makedirs(os.path.join(dpath, f'LP4Rec/filter/'), exist_ok=True)
    # torch.save(stu_ks, os.path.join(dpath, f"LP4Rec/filter/top1_pkc.pth"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default=f"examples/saved_model/{save_model_path}")
    ...
    print(args)
    params = vars(args)
    main(params, testmodel)

    # df = pd.read_csv('E:\zjh\pykt-toolkit-main\data/nips_task34\model\LP4Rec/test_sequences.csv')
    # df = df['responses']
    # for idx, item in df.items():
    #     if len(item.split(',')) != 200:
    #         print(idx)


    # df = df.apply(lambda x: [int(c) for c in x.split(',') if c != '-1'])
    # m = 0
    # for idx, item in df.items():
    #     m = max(m, max(item))
    # print(m)